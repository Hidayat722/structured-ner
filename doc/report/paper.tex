...
\ex. \textbf{x:} The motor company 'Ford' was founded and incorporated by Henry Ford.\\ \textbf{y:} O  O  ORG  O  O O O O PER PER.  \label{PredEx1b} 
...
Learning is implemented with a Structured Perceptron. To avoid overfitting, we average the parameters after the last iteration. For finding the best sequence of labels, we use a modified version of the Viterbi algorithm. The algorithm finds the sequence $\hat{y}$, such that:
