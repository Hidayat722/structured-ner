...
\begin{abstract}
In this paper, we apply the method of structure prediction to the information extraction task Named Entity Recognition (NER). We illustrate a number issues relevant to the NER task, describe our implementation and compare it to state-of-the art methods.
...
Apart from the above, there is also the TIMEX type, which covers date and time expressions and \emph{NUMEX} for monetary values and percentages.
...
There exist two general approaches: one is rule-based and the other statistical. 
Rule-based methods make use of the underlying rules governing a language to extract Named Entities. 
This approach, however, is high in maintenance and requires extensive work by experts, such as computational linguists \cite{nadeau2007survey}.
The results can have high precision, but generally lack considerably in recall. % rules are partly language-dependent
...
\todo{Ich würde hier eher den F-Score nennen.}This system achieves a precision of 88.99\% on the English test set and 83.87\% German.
...
last column of the trellis with maximal score $\delta_n(a)$. From this, the sequence can be recovered via back-tracking through the trellis. 
...
Thus, for the sequence: ``Jack London went to New York'' the NE tag combination of \emph{PER} and \emph{PER} in example~\ref{seq1} is more likely than\emph{PER} and \emph{LOC} in \ref{seq2}.
